...
def create_advanced_features(df):
    """Create features that properly account for division gaps"""
    df = df.copy()
    ...
    # Form features weighted by division gap
...
def predict_with_division_rules(model, X, division_gap_threshold=3):
    """Make predictions with special handling for large division gaps"""
    ...
    for i, (pred, gap) in enumerate(zip(y_pred_raw, division_gaps)):
        if gap >= division_gap_threshold:
            # For large division gaps, only trust very confident predictions
            if probs[i][0] > 0.7:  # Very confident home win
                y_pred.append(0)
            elif probs[i][2] > 0.7:  # Very confident away win
                y_pred.append(2)
            else:
                # Default to higher division team
                y_pred.append(0 if X.iloc[i]['DivisionGap'] < 0 else 2)
        else:
            # Normal prediction for small division gaps
